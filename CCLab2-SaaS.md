...
- 在Windows服务器上配置Remote App，并尝试发布App
- 在Docker服务器上部署Guacamole（推荐），或使用native方式部署Guacamole
- 使用Guacamole访问Windows桌面App

#### 可选任务

> 作为思考题，可以将你的解决思路写在在实验报告中。不是必须要完成的部分，更不要求具体实现。提示：可以通过bat脚本、网络磁盘映射、组策略限制等等方式来解决问题。

- 回顾我们部署的SaaS“平台”，也就是Guacamole。很显然，它无法正常作为一个平台来向用户提供服务，因为我们手动输入了Windows账号来添加一个App。对于一个平台来说，手动为用户分配账号显然是不现实的。这个问题如何解决？
- 回顾我们访问的画图应用。在存储文件时，可以无限制地将文件存储在任何地方。同样，这样也十分不合理，因为不同用户可以互相读取他人的文件是不可接受的。此外，对于Windows服务器来说，随意更改C盘关键位置的某些文件也是十分危险的。这个问题如何解决？
- 在解决了前两个问题的前提下，如果用户恶意调出了cmd/powershell对服务器设置进行更改，如何进行阻拦？